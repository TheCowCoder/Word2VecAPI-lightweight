...
from numpy import exp, dot, zeros, outer, random, dtype, get_include, float32 as REAL, uint32, seterr, array, uint8, vstack, argsort, fromstring, sqrt, newaxis, ndarray, empty, sum as np_sum
import pickle
import argparse
import base64
import sys
...
class N_Similarity(Resource):
  ...
  def get(self):
    ...
    parser.add_argument("ws2",
                        type=str,
                        required=True,
                        help="Word set 2 cannot be blank!",
                        action="append")
    args = parser.parse_args()
    return model.n_similarity(filter_words(args["ws1"]),
                              filter_words(args["ws2"])).item()
...
class MostSimilar(Resource):
  ...
  def get(self):
    ...
    print("positive: " + str(pos) + " negative: " + str(neg) + " topn: " +
          str(t))
    try:
...
if __name__ == "__main__":
  ...
  print("Loading model...", model_path, binary)
  # model = word2vec.KeyedVectors.load_word2vec_format(model_path, binary=binary)
  model = genapi.load("word2vec-google-news-300"
